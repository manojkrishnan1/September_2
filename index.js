...
let tab1 = document.getElementById('table-1');
let tab2 = document.getElementById('table-2');
let table2 = document.getElementById('db2');    //div of table-2 - Display is toggled ON only when sort Gender is used.

addStudents(students, tab1);

function clearStudents(tab){    
    let childElements = tab.children.length;

    while(childElements>1){
        tab.removeChild(tab.lastElementChild);
        childElements--;
    }
}

function addStudents(arr, tab){
    ...
    for(let t=0; t<n; t++){
        ...
        //Temporary row 'tr' for adding each person
        let temp_row = document.createElement('tr');

        //Creating 7 'td' for adding the properties for each person
        let temp_cell = [];
        for(let i=0; i<7; i++){
        ...
        //Getting the values from Objects in the Array to the 'td'
        temp_cell[0].innerHTML =  arr[t]['id'];
        temp_cell[1].innerHTML =  arr[t]['first_name'] + " " + arr[t]['last_name'];
        temp_cell[2].innerHTML =  arr[t]['gender'] ;
        temp_cell[3].innerHTML =  arr[t]['class'] ;
        temp_cell[4].innerHTML =  arr[t]['marks'] ;
        temp_cell[5].innerHTML =  arr[t]['passing'] ? "Passing" : "Failed";  
        temp_cell[6].innerHTML =  arr[t]['email'] ;


        for(let i=0; i<7;i++){
            temp_row.appendChild(temp_cell[i]);
        }

        tab.appendChild(temp_row);


        let image = document.createElement('img');
        image.src = arr[t]['img_src'];
        let cellid = tab.rows[t+1].children[1];
        cellid.prepend(image);
    }
}

function searchStudents(){
    let searchElement = document.getElementById('searchString');

    let str = searchElement.value.toLowerCase().trim();

    let result = [];

    for(let i=0; i<100; i++){
        let x = students[i];
        if(x['first_name'].toLowerCase().includes(str) || x['last_name'].toLowerCase().includes(str) || x['email'].toLowerCase().includes(str)){
           result.push(x);
        }
    }
    addStudents(result, tab1);
}

function sortAZ(){
    let result = [...students];
    result.sort(({first_name: a}, {first_name: b})=>{
        if(a<b){
            return -1;
        }
        else if(a>b){
...
function sortMarks(){
    ...
    result.sort(({marks: a}, {marks: b}) => a-b);
...
function sortClass(){
    ...
    result.sort(({class: a}, {class: b}) => a-b);
...
function sortGender(){
    ...
    addStudents(male, tab1);
    addStudents(female, tab2);
}
